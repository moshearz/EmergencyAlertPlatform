...
int main(int argc, char *argv[]) {
    ...
    //This thread should waits until user logged in, once it happaning this thread become active and starts listening for messages from the server
        ...
        while (!shouldTerminate) {
            ...
            cv.wait(lock, [&]() { return loginInProgress || shouldTerminate; });
            ...
            try {
                ...
                if (!connectionhandler->getLine(msg)) {
                    ...
                    std::lock_guard<std::mutex> terminateLock(mutex);
                ...
                if (msg.find("CONNECTED") == 0) {
                    ...
                    cv.notify_all();
                    std::cout << "DEBUG: CONNECTED frame processed, login successful." << std::endl;
                } else if (msg.find("ERROR") == 0) {
                    std::lock_guard<std::mutex> lock(mutex);
                    loginInProgress = false;
                    cv.notify_all();
                    std::cerr << "DEBUG: ERROR frame received: " << msg << std::endl;
                }
    ...
    std::thread KeyboardThread([&stompProtocol, &connectionhandler, &shouldTerminate, &mutex, &isLoggedIn, &loggedInUsername, &subscriptionMap, &eventsMap, &cv]() {
        ...
        while(!shouldTerminate){
            ...
            if(userInput.rfind("login ", 0) == 0){
                ...
                while (!connectionhandler->getLine(serverResponse)) {
                    ...
                    if (std::chrono::duration_cast<std::chrono::seconds>(elapsedTime).count() > 10) { // Timeout after 10 seconds
                            std::cerr << "DEBUG: Timeout while waiting for server response." << std::endl;
                            delete connectionhandler;
                            delete stompProtocol;
                            connectionhandler = nullptr;
                            stompProtocol = nullptr;
            ...
            std::cout << "DEBUG: Received server response:\n" << serverResponse << std::endl;
                    ...
                    std::cout << "Login successful" << std::endl;
                     {
                        ...
                        cv.notify_all(); // Notify the serverCommunicationThread
                    }
